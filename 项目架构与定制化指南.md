...
> 本文档详细分析 cwgo 项目的架构设计，并提供完整的定制化开发指导。
...
10. [最佳实践](#十最佳实践)
11. [附录](#十一附录)
12. [cwgo 与 Eino 框架集成分析](#十二cwgo-与-eino-框架集成分析)
...
### 6.5 高级定制：Service 编排与 Logic 算子化

#### 6.5.1 核心理念

为了解决复杂业务场景下 Service 层代码臃肿、难以测试的问题，推荐采用**逻辑编排与算子化**的设计模式。

核心思想是将 Service 层做薄，仅负责策略分发；将业务逻辑下沉为无状态的原子算子（Logic）；通过独立的 Strategy 层在 `init` 阶段完成逻辑编排（串行或 DAG）。

#### 6.5.2 架构分层

| 层级 | 职责 | 生成策略 | 说明 |
|------|------|----------|------|
| **Service 层** | **分发** | `Cover` (覆盖) | 定义接口标准和全局策略 Map，运行时查表执行，不包含具体逻辑。 |
| **Strategy 层** | **编排** | `Skip` (不覆盖) | 用户在此编写 `init` 函数，将 Logic 算子组装成执行流，注册到 Map 中。 |
| **Logic 层** | **执行** | 不生成 | 用户自研的原子业务逻辑，纯 Go 代码，与框架解耦。 |
...
│   └── user/
│       └── get_user.go   
├── strategy/             # [自动生成一次] 策略编排层，用户在此处由 init 组装 Logic
│   └── user/
│       └── get_user.go   
└── logic/                # [用户自研] 原子业务逻辑，无代码生成干扰
    └── user/
        ├── check_permission.go
        └── query_db.go
```
...
func (s *GetUserService) Run(req *user.GetUserRequest) (resp *user.GetUserResponse, err error) {
    // 默认使用 "default" 策略，也可以根据 req 动态选择
...
import (
    ...
    "example/biz/logic/user_logic" // 导入 Logic 算子
)
...
func init() {
    ...
    user_service.GetUserStrategies["default"] = func(ctx context.Context, req *user.GetUserRequest) (*user.GetUserResponse, error) {
        ...
        // 1. 算子 A: 检查权限
        if err := user_logic.CheckPermission(ctx, req.UserId); err != nil {
            return nil, err
        }
        
        // 2. 算子 B: 查询数据
        userInfo, err := user_logic.QueryUserInfo(ctx, req.UserId)
        if err != nil {
            return nil, err
        }
...
```go
// biz/logic/user/check_permission.go
package user_logic
...
1. **逻辑解耦**：Logic 层完全不知道 Kitex/Hertz 的存在，可以轻松被 HTTP 和 RPC 接口同时复用。
2. **代码隔离**：Service 层文件可以随时重新生成，不会覆盖用户手写的编排逻辑。
3. **编排灵活**：在 `init` 中可以使用 Go 代码自由控制流程（如并发、重试、事务），不受模板限制。
4. **易于测试**：测试时可以向 `Strategies` Map 注入 Mock 实现，无需启动真实依赖。
